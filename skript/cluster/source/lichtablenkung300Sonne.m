...
M = 300*Msun;            % Linsenmasse
deg = atan(Rsun/xc);   % Blickrichtung


diffSy = @(l,x) [x(2);
                 (4*x(2)^2*x(3)*K*M) / (c^2*sqrt( x(3)^2+(x(1)-xc)^2 )^3) ...
                 - (2* (x(4)^2-x(2)^2) * (x(1)-xc) *K*M) / ...
                   (c^2 *sqrt(x(3)^2+(x(1)-xc)^2)^3);
                 x(4);
                 (4*x(2)*x(4)*(x(1)-xc)*K*M) / (c^2*sqrt(x(3)^2+(x(1)-xc)^2)^3) ...
...
yy(1,:) = y(:,1);
yy(2,:) = y(:,3);
csvwrite('lichtablenkung300Sonne.csv', yy');
