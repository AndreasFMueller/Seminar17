...
Wir verlangen daher, dass die Parameterisierung genügend oft nach
...
in Teilstrecken aufteilen und die Längen der Teilstrecken
aufaddieren.
Die Summe
\[
...
dass $|\dot c(t)|\ne 0$ ist für alle Parameterwerte $t$, dann ist
die Funktion
$s(t)$
...
\centering
\begin{tikzpicture}[]
\node at (0,0) {\includegraphics[width=\hsize]{chapters/3d/kurve.jpg}};
\node at (1.1,4.5) {$z$};
\node at (-0.9,1.3) {$x$};
\node at (-3.4,-0.1) {$y$};
\node at (-2.4,3) {$c_2(t)$};
\node at (-0.9,-0.6) {$c_1(t)$};
\end{tikzpicture}
\caption{Kurven $t\mapsto(t,t^2,0)$ (rot) und $t\mapsto(t,t^2,t^3)$ (gelb).
\label{skript:kruemmung:fig:kurvekr}}
\end{figure}
Wir betrachten die beiden Raumkurven
\begin{align*}
c_1(t)&=(t,t^2,0)
&
c_2(t)&=(t,t^2,t^3)
