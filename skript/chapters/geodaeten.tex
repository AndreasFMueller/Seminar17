...
indem man die Fläche in eine Ebene abrollt, und dann verwendet,
...
mit Ebenen durch die zwei gegebenen Punkte ist der Grosskreis derjenige
...
\label{skript:geodaeten:fig:transport}}
\end{figure}
...
zu halten (Abbildung~\ref{skript:geodaeten:fig:transport}).
...
\Gamma^\mu_{\alpha\nu}\dot x^\alpha
\]
sein und die zeitliche Änderung entlang der Kurve wird
\[
...
die Summationskonvention als Lauf{}indizes ausgezeichneten Indizes verschwindet.
Wir erhalten also
\begin{equation}
\frac{\partial g_{\mu\nu}}{\partial x^\alpha}
-
...
heben sich die ersten Terme weg, es bleibt
\[
g_{\alpha\beta}\Gamma_{\nu\mu}^\beta
=
\biggl(
...
\end{definition}
\index{Christoffelsymbole erster Art}%
\index{Christoffelsymbole zweiter Art}%
...
die Definition \eqref{skript:definition:christoffel2} und erhalten
\begin{align*}
-\frac{\partial g_{\mu\nu}}{\partial x^\alpha} g^{\mu\sigma}
+
g_{\mu\nu}
...
+
\dotsc,
\label{skript:geodaeten:kovablbel}
\end{equation}
...
Aus der Formel~\eqref{skript:geodaeten:kovablbel} können wir jetzt eine
...
Ableitung ist nach \eqref{skript:geodaeten:kovablbel}
\begin{equation}
\nabla_\alpha T^\mu_\nu
=
\frac{\partial T^\mu\nu}{\partial x^\alpha}
...
Mit Hilfe der Formel~\eqref{skript:geodaeten:kovablbel} erlaubt uns nun
...
und zweiter Art für Zy\-lin\-der- und Kugeloberfläche berechnen.

\rhead{Beispiele}

\subsection{Polarkoordinaten}
Die nicht verschwindenden Komponenten
...
Da die Komponenten des metrischen Tensors konstant sind, verschwinden
alle Ableitungen
\[
\frac{\partial g_{\mu\nu}}{\partial x^\alpha}=0
\qquad\Rightarrow\qquad
...
\label{skript:kruemmung:geodaetengleichung}
\end{equation}
\index{Geodäte}%

\begin{definition}
Eine Kurve in einem Riemannschen Raum heisst eine Geodäte, wenn sie
die Differentialgleichung~\eqref{skript:kruemmung:geodaetengleichung}
erfüllt.
\end{definition}
...
in
\eqref{skript:kruemmung:geodaetengleichung}.
...
$\varphi=0$-Achse, in $x$-$y$-Koordinaten können wir sie als
...
Als nächstes betrachten wir die {\em Meridiane} der Kugel.
\index{Meridian}%
...
Wenn die Vermutung stimmt, müssten die Euler-Lagrange-Gleichun\-gen 
...
die Gleichungen~\eqref{skript:kruemmung:geodaetengleichung}
