...
\]
schreiben, und die beiden Werte stimmen überein.
...
Sollte dies nicht möglich sein, muss das Gebiet in Teilgebiete zerlegt
werden, für die dies möglich ist.
Die Wegintegrale über die gemeinsamen Teile des Ränder der Teilgebiet 
heben sich weg.
...
Sei also $A_\mu$ ein Vektorfeld und $x^\mu(t)$ eine Kurve.
...
Der Vektor $A^\nu$ würde dann entlang des Parallelogramms um den
Vektor
\[
R^\mu\mathstrut_{\nu\alpha\beta} A^\nu u^\alpha v^\beta
\]
...
\Delta A_\mu
=
\oint \nabla_\sigma A_\mu\,dx^\sigma
=
\oint \frac{\partial A_\mu}{\partial x^\sigma}
+
\Gamma^\nu_{\mu\sigma} A_\nu \,x^\sigma
= 
...
Zusätzlich zum eben berechneten Riemann-Tensor wird oft auch ein
...
Man beachte, dass der Ricci-Krümmungsskalar, der im
Kapitel~\ref{skript:chapter:gravitation} eingeführt wird,
konstant ist.
Damit wird ausgedrückt, dass die Kugel konstante Krümmung hat.
Die Komponenten des Riemann-Tensors sind dagegen nicht konstant,
da sie von der Wahl des Koordinatensystems abhängen.
...
Wir werden später in Abschnitt~\ref{skript:speziell:kraefte}
sehen, dass die Christoffelsymbole physikalisch die
