...
$x=R\cos t$ und $y=R\sin t$. In einer Ebene wird
der Flächeninhalt $F=\pi R^2$ sein, die Umfang $U=2\pi R$.
...
verwenden dabei ein Koordinatensystem, in dem die Hauptrkümmungsrichtungen
die Achsen sind.
...
ziehen können.
Da wir aber ohnehin nur mit einer Approximation für kleine Werte von $r$
arbeiten, können wir den Integranden in eine Taylorreihe in $r$ entwickeln.
Die Rechnung mit dem Maxima Programm
\lstinputlisting[style=Maxima]{chapters/listings/flaecheninhalt.m}
ergibt
\begin{align*}
F
%&=
...
=
\begin{pmatrix}
R\cos t\\ R\sin t\\ R^2(\kappa_1\cos^2 t + \kappa_2^2\sin^2 t)
\end{pmatrix}
=
R
\begin{pmatrix}
\cos t\\ \sin t\\ R(\kappa_1\cos^2 t + \kappa_2^2\sin^2 t)
\end{pmatrix}
\]
...
\sqrt{
\sin^2 t + \cos^2 t 
+ R^2(\kappa_1^2 \cos^2 t + \kappa_2\sin^2 t)
}
\,dt,
\end{align*}
...
Die Rechnung mit dem Maxima Programm
\lstinputlisting[style=Maxima]{chapters/listings/umfang.m}
gibt wieder
\begin{align*}
U
&\simeq
...
1
+ 
\frac12 (\kappa_1^2 -2\kappa_1\kappa_2+\kappa_2^2) R^2
).
\end{align*}
...
\frac{4\pi F}{U^2}
&=
\frac{4\pi^2R^2(1+(\kappa_1^2 + \kappa_2^2)R^2)}%
{4\pi^2 R^2(1+(\kappa_1^2-\kappa_1\kappa_2+\kappa_2^2)R^2)}
=
\frac{1+(\kappa_1^2 + \kappa_2^2)R^2}%
{1+(\kappa_1^2-\kappa_1\kappa_2+\kappa_2^2)R^2}.
