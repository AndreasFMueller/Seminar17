...
Offenbar ist der sichtbare Teil der Milchstrasse nicht einmal in der Ebene
der Milchstrasse isotrop.
...
der Druck des Gases ist also sehr gering.
...
Stösse praktisch nicht mehr vor.
...
haben wir gesehen, dass die letzten zwei Terme zusammengefasst
werden müssen.
Wir können dies dadurch ausdrücken, dass wir die zwei Terme
\[
d\Omega^2 = d\vartheta^2 + \sin^2\vartheta\,d\varphi^2
\]
zusammennehmen müssen, um eine isotrope Metrik zu erhalten.
...
Insbesondere muss eine von der euklidischen abweichende
...
(Siehe auch die Definition in 
Abschnitt~\ref{skript:gravitation:einstein-tensor}).
...
\]
haben.
Einzig der Parameter $\kappa$ und der Krümmungsradius $R_c$ sind
noch wählbar. 
...
\end{align*}
Durch Gleichsetzen bekommen wir eine Beziehung zwischen Druck und
Dichte.
Diese zweite Gleichung würde also die Temperatur des Universums 
...
Fläche der letzten Streuung verwendet werden, von der der
Mikrowellenhintergrund ausgestrahlt wurde.
Für diesen Zweck zur Verfügung stehen Fluktuationen im kosmischen
Mikrowellenhintergrund (siehe auch das Umschlagbild).
